...
/* Set marked pane. */
void
server_set_marked (struct session *s, struct winlink *wl,
...
int
server_is_marked (struct session *s, struct winlink *wl,
		  struct window_pane *wp)
{
...
/* Tidy up every hour. */
static void
server_tidy_event (__unused int fd, __unused short events,
		   __unused void *data)
{
