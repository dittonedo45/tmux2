...
void
key_bindings_add (const char *name, key_code key, const char *note,
		  int repeat, struct cmd_list *cmdlist)
{
  ...
  log_debug ("%s: %#llx %s = %s", __func__, bd->key,
	     key_string_lookup_key (bd->key, 1), cmd_list_print (bd->cmdlist,
								 0));
}
...
key_bindings_init (void)
{
  static const char *defaults[] = {
    ...
    /* Menu keys */
    "bind < display-menu -xW -yW -T '#[align=centre]#{window_index}:#{window_name}' "
      DEFAULT_WINDOW_MENU,
...
key_bindings_dispatch (struct key_binding *bd, struct cmdq_item *item,
		       struct client *c, struct key_event *event,
