...
static
TAILQ_HEAD (, window)
  alerts_list = TAILQ_HEAD_INITIALIZER (alerts_list);
...
alerts_check_session (struct session *s)
{
  ...
  RB_FOREACH (wl, winlinks, &s->windows) alerts_check_all (wl->window);
}
...
alerts_reset_all (void)
{
  ...
  RB_FOREACH (w, windows, &windows) alerts_reset (w);
}
...
alerts_set_message (struct winlink *wl, const char *type, const char *option)
{
  ...
  TAILQ_FOREACH (c, &clients, entry)
  {
    ...
    if (c->session->curw == wl)
      {
	status_message_set (c, -1, 1, 0, "%s in current window", type);
      }
    else
      {
	status_message_set (c, -1, 1, 0, "%s in window %d", type, wl->idx);
      }
  }
}
