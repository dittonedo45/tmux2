...
static int window_copy_search_jump (struct window_mode_entry *, struct grid *,
				    struct grid *, u_int, u_int, u_int, int,
				    int, int, int);
...
window_copy_search_rl (struct grid *gd,
		       struct grid *sgd, u_int * ppx, u_int py, u_int first,
		       u_int last, int cis)
{
  u_int ax, bx, px, pywrap, endline;
  int matched;
...
static int
window_copy_search_lr_regex (struct grid *gd, u_int * ppx, u_int * psx,
			     u_int py, u_int first, u_int last, regex_t * reg)
{
  int eflags = 0;
...
static int
window_copy_search_rl_regex (struct grid *gd, u_int * ppx, u_int * psx,
			     u_int py, u_int first, u_int last, regex_t * reg)
{
  int eflags = 0;
...
window_copy_last_regex (struct grid *gd, u_int py, u_int first, u_int last,
			u_int len, u_int * ppx, u_int * psx, const char *buf,
...
static void
window_copy_cstrtocellpos (struct grid *gd, u_int ncells, u_int * ppx,
...
window_copy_search_jump (struct window_mode_entry *wme, struct grid *gd,
			 struct grid *sgd, u_int fx, u_int fy, u_int endline,
			 int cis, int wrap, int direction, int regex)
{
  ...
  if (found)
    {
	  ...
	  at > 0 && data->searchmark[at] == data->searchmark[at - 1])
	{
	  window_copy_move_after_search_mark (data, &fx, &fy, wrapflag);
...
window_copy_write_one (struct window_mode_entry *wme,
		       struct screen_write_ctx *ctx, u_int py, u_int fy,
		       u_int nx, const struct grid_cell *mgc,
		       const struct grid_cell *cgc,
		       const struct grid_cell *mkgc)
{
  struct window_copy_mode_data *data = wme->data;
  ...
  if (py == 0 && s->rupper < s->rlower && !data->hide_position)
      ...
      if (data->searchmark == NULL)
	{
	  if (data->timeout)
	    ...
	    {
	      size = xsnprintf (hdr, sizeof hdr, "[%u/%u]", data->oy, hsize);
	    }
	}
      else
	{
				    ...
				    int no_reset)
{
  ...
  switch (data->selflag)
      ...
      if (data->dy > yy || (data->dy == yy && data->dx > xx))
	{
	  ...
	  window_copy_cursor_previous_word_pos (wme, data->ws, 0, &xx, &yy);
	  begin = 1;
      ...
      else
	{
	  ...
	  if (xx >= window_copy_find_length (wme, yy) ||
	    ...
	    window_copy_cursor_next_word_end_pos (wme, data->ws, &xx, &yy);
  ...
  if (grid_reader_cursor_jump (&gr, data->jumpchar))
    {
      ...
      window_copy_acquire_cursor_down (wme, hsize,
				       screen_size_y (back_s), data->oy, oldy,
				       px, py, 0);
    }
}
...
window_copy_cursor_next_word_end_pos (struct window_mode_entry *wme,
				      const char *separators, u_int * ppx,
				      u_int * ppy)
{
  struct window_pane *wp = wme->wp;
  struct window_copy_mode_data *data = wme->data;
...
window_copy_start_drag (struct client *c, struct mouse_event *m)
{
  ...
  switch (data->selflag)
	...
	{
	  window_copy_update_cursor (wme, x, y);
	  window_copy_cursor_previous_word_pos (wme, data->ws, 0, &x, &y);
...
window_copy_acquire_cursor_down (struct window_mode_entry *wme, u_int hsize,
				 u_int sy, u_int oy, u_int oldy, u_int px,
				 u_int py, int no_reset)
{
  u_int cy, yy, ny, nd;
