...
ar_Value *ar_to_string_value(ar_State * S, ar_Value * v, int quotestr)
{
	...
	while (v) {
	    if (v->type == AR_TPAIR) {
		last =
		    ar_append_tail(S, last,
				   ar_to_string_value(S, ar_car(v), 1));
		if (ar_cdr(v)) {
		...
		last = ar_append_tail(S, last, ar_new_string(S, ". "));
		last =
		    ar_append_tail(S, last, ar_to_string_value(S, v, 1));
	    }
	    v = ar_cdr(v);
	}
	last = ar_append_tail(S, last, ar_new_string(S, ")"));
...
void ar_gc(ar_State * S)
{
    ...
    ar_Chunk *c;
    /* Mark roots */
    for (i = 0; i < S->gc_stack_idx; i++)
	ar_mark(S, S->gc_stack[i]);
    ar_mark(S, S->global);
...
static ar_Value *get_bound_value(ar_Value * sym, ar_Value * env)
{
	...
	if (x)
	    return x->u.map.pair->u.pair.cdr;
...
ar_Value *ar_bind(ar_State * S, ar_Value * sym, ar_Value * v,
		  ar_Value * env)
{
    ar_Value **x = get_map_ref(&env->u.env.map, sym);
    if (*x) {
...
ar_Value *ar_set(ar_State * S, ar_Value * sym, ar_Value * v,
		 ar_Value * env)
{
    for (;;) {
	ar_Value *x = *get_map_ref(&env->u.env.map, sym);
	if (x)
	    return x->u.map.pair->u.pair.cdr = v;
	if (!env->u.env.parent)
...
static ar_Value *args_to_env(ar_State * S, ar_Value * params,
			     ar_Value * args, ar_Value * env)
{
...
static void pop_frame(ar_State * S, ar_Value * rtn)
{
    ...
    if (S->frame == &S->base_frame)
	S->gc_stack_idx = 0;
    if (rtn)
	push_value_to_stack(S, rtn);
}


static ar_Value *raw_call(ar_State * S, ar_Value * caller, ar_Value * fn,
			  ar_Value * args, ar_Value * env)
{
    ar_Value *e, *res;
    ar_Frame frame;
...
ar_Value *ar_eval(ar_State * S, ar_Value * v, ar_Value * env)
{
    ...
    case AR_TCFUNC:
    case AR_TFUNC:
	args = eval_list(S, v->u.pair.cdr, env);
	break;
    default:
	args = v->u.pair.cdr;
	break;
...
static ar_Value *p_and(ar_State * S, ar_Value * args, ar_Value * env)
{
    ar_Value *res = NULL;
    while (args) {
	if (!(res = ar_eval(S, ar_car(args), env)))
	    return NULL;
	args = ar_cdr(args);
    }
    return res;
...
{
    return ar_check(S, ar_car(args), AR_TPAIR)->u.pair.car =
	ar_nth(args, 1);
}
...
{
    return ar_check(S, ar_car(args), AR_TPAIR)->u.pair.cdr =
	ar_nth(args, 1);
}
...
static ar_Value *f_string(ar_State * S, ar_Value * args)
{
    ...
    while (v) {
	last =
	    ar_append_tail(S, last, ar_to_string_value(S, ar_car(v), 0));
	v = ar_cdr(v);
    }
    return join_list_of_strings(S, res);
}
...
static ar_Value *f_strlen(ar_State * S, ar_Value * args)
{
    return ar_new_number(S,
			 ar_check(S, ar_car(args), AR_TSTRING)->u.str.len);
}
...
static ar_Value *f_loads(ar_State * S, ar_Value * args)
{
    ...
    if (!fp)
	ar_error_str(S, "could not open file");
    /* Get size */
    fseek(fp, 0, SEEK_END);
    size = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    ...
    fclose(fp);
    if (r != size)
	ar_error_str(S, "could not read file");
    return res;
}
...
static ar_Value *f_dumps(ar_State * S, ar_Value * args)
{
    ...
    data =
	ar_to_stringl(S, ar_check(S, ar_nth(args, 1), AR_TSTRING), &len);
    ...
    fclose(fp);
    if (r != 1)
	ar_error_str(S, "could not write file");
    return NULL;
}
...
static ar_Value *f_mod(ar_State * S, ar_Value * args)
{
    ...
    if (b == 0.)
	ar_error_str(S, "expected a non-zero divisor");
