  ...
  if (strcmp (path, "-") == 0)
    {
      ...
      fd = STDOUT_FILENO;
      if (c == NULL ||
	  (c->flags & CLIENT_ATTACHED) || (c->flags & CLIENT_CONTROL))
	{
	  cf->error = EBADF;
	  goto done;
	}
...
/* Read a file. */
void
file_read (struct client *c, const char *path, client_file_cb cb,
	   void *cbdata)
{
  struct client_file *cf;
...
static void
file_write_error_callback (__unused struct bufferevent *bev,
...
static void
file_read_error_callback (__unused struct bufferevent *bev,
			  __unused short what, void *arg)
{
  struct client_file *cf = arg;
