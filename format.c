/*XXX This Document was modified on 1647343832 */
/* $OpenBSD$ */
...
static void
printflike ( 3, 4 )
format_log1 ( struct format_expand_state *es, const char *from,
              const char *fmt, ... )
{
...
/* Find a job. */
static char *format_job_get ( struct format_expand_state *es, const char *cmd )
{
 struct format_tree *ft = es->ft;
 ...
 if( force || ( fj->job == NULL && fj->last != t ) ) {
  fj->job = job_run ( expanded, 0, NULL, NULL,
                      server_client_get_cwd ( ft->client, NULL ),
...
static char *printflike ( 1, 2 ) format_printf ( const char *fmt, ... )
{
 va_list ap;
 char *s;
...
static void *format_cb_pane_in_mode ( struct format_tree *ft )
{
 ...
 TAILQ_FOREACH ( wme, &wp->modes, entry ) n++;
 xasprintf ( &value, "%u", n );
 return ( value );
}
...
{
 if( ft->c != NULL &&
     ft->c->last_session != NULL && session_alive ( ft->c->last_session ) )
...
static void *format_cb_config_files ( __unused struct format_tree *ft )
{
 char *s = NULL;
 size_t slen = 0;
 u_int i;
...
static void *format_cb_socket_path ( __unused struct format_tree *ft )
{
 return ( xstrdup ( socket_path ) );
}

/* Callback for version. */
static void *format_cb_version ( __unused struct format_tree *ft )
{
...
static void *format_cb_start_time ( __unused struct format_tree *ft )
{
 return ( &start_time );
}
...
static void *format_cb_buffer_mode_format ( __unused struct format_tree *ft )
{
 return ( xstrdup ( window_buffer_mode.default_format ) );
}

/* Callback for client_mode_format, */
static void *format_cb_client_mode_format ( __unused struct format_tree *ft )
{
 return ( xstrdup ( window_client_mode.default_format ) );
}

/* Callback for tree_mode_format, */
static void *format_cb_tree_mode_format ( __unused struct format_tree *ft )
{
...
static char *format_find ( struct format_tree *ft, const char *key,
                           int modifiers, const char *time_format )
{
...
/* Build modifier list. */
static struct format_modifier *format_build_modifiers ( struct
 ...
 *count = 0;
 ...
 while( *cp != '\0' && *cp != ':' ) {
  ...
  if( strchr ( "labdnwETSWP<>", cp[0] ) != NULL && format_is_end ( cp[1] ) ) {
  ...
  if( ( memcmp ( "||", cp, 2 ) == 0 ||
        ...
        memcmp ( "==", cp, 2 ) == 0 ||
        memcmp ( "<=", cp, 2 ) == 0 ||
        memcmp ( ">=", cp, 2 ) == 0 ) && format_is_end ( cp[2] ) ) {
...
/* Loop over sessions. */
static char *format_loop_sessions ( struct format_expand_state *es,
                                    const char *fmt )
{
 struct format_tree *ft = es->ft;
...
/* Loop over windows. */
static char *format_loop_windows ( struct format_expand_state *es,
                                   const char *fmt )
{
 struct format_tree *ft = es->ft;
 struct client *c = ft->client;
...
/* Loop over panes. */
static char *format_loop_panes ( struct format_expand_state *es,
                                 const char *fmt )
{
 struct format_tree *ft = es->ft;
 struct client *c = ft->client;
...
/* Replace a key. */
static int
format_replace ( struct format_expand_state *es, const char *key,
...
static char *format_expand1 ( struct format_expand_state *es, const char *fmt )
{
 struct format_tree *ft = es->ft;
 char *buf, *out, *name;
 const char *ptr, *s;
...
char *format_single_from_target ( struct cmdq_item *item, const char *fmt )
{
 ...
 return ( format_single_from_state
          ( item, fmt, tc, cmdq_get_target ( item ) ) );
}
