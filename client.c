...
static int client_connect (struct event_base *, const char *, uint64_t);
	     ...
	     int feat)
{
   ...
   */
  if (pledge ("stdio rpath wpath cpath unix sendfd proc exec tty", NULL) != 0)
    fatal ("pledge failed");

  /* Load terminfo entry if any. */
  if (isatty (STDIN_FILENO) &&
      *termname != '\0' &&
      tty_term_read_list (termname, STDIN_FILENO, &caps, &ncaps, &cause) != 0)
    {
      fprintf (stderr, "%s\n", cause);
      free (cause);
      return (1);
...
client_file_check_cb (__unused struct client *c, __unused const char *path,
		      __unused int error, __unused int closed,
		      __unused struct evbuffer *buffer, __unused void *data)
{
  if (client_exitflag)
    client_exit ();
}
...
client_dispatch_wait (struct imsg *imsg)
{
  ...
  if (!pledge_applied)
    {
      if (pledge ("stdio rpath wpath cpath unix proc exec tty", NULL) != 0)
	fatal ("pledge failed");
      pledge_applied = 1;
    }
  ...
  switch (imsg->hdr.type)
	...
	fatalx ("bad MSG_FLAGS string");
      ...
      log_debug ("new flags are %#llx", (unsigned long long) client_flags);
      break;
    case MSG_SHELL:
