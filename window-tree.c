...
window_tree_build_pane (struct session *s, struct winlink *wl,
			struct window_pane *wp, void *modedata,
		   ...
		   uint64_t * tag, const char *filter)
{
  ...
  {
    if (data->squash_groups && (sg = session_group_contains (s)) != NULL)
      {
...
window_tree_draw_label (struct screen_write_ctx *ctx, u_int px, u_int py,
			u_int sx, u_int sy, const struct grid_cell *gc,
			const char *label)
{
  size_t len;
  u_int ox, oy;
...
static void
window_tree_draw_session (struct window_tree_modedata *data,
			  struct session *s, struct screen_write_ctx *ctx,
			  u_int sx, u_int sy)
{
...
window_tree_draw_window (struct window_tree_modedata *data, struct session *s,
			 struct window *w, struct screen_write_ctx *ctx,
			 u_int sx, u_int sy)
{
  struct options *oo = s->options;
  struct window_pane *wp;
...
static void
window_tree_draw (void *modedata, void *itemdata,
		  ...
		  struct args *args)
{
  ...
  data->data = mode_tree_start (wp, args, window_tree_build,
				window_tree_draw, window_tree_search,
				window_tree_menu, NULL, window_tree_get_key,
				data, window_tree_menu_items,
