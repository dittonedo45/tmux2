...
control_append_data (struct client *c, struct control_pane *cp, uint64_t age,
		     struct evbuffer *message, struct window_pane *wp,
		     size_t size)
{
  u_char *new_data;
  size_t new_size;
...
static int
control_write_pending (struct client *c, struct control_pane *cp,
		       size_t limit)
{
  struct control_state *cs = c->control_state;
...
control_discard (struct client *c)
{
  ...
  RB_FOREACH (cp, control_panes, &cs->panes) control_discard_pane (c, cp);
...
/* Add a subscription. */
void
control_add_sub (struct client *c, const char *name,
