...
static void
printflike (2, 3)
input_reply (struct input_ctx *, const char *, ...);
...
/* Input state handlers. */
     ...
     static void input_csi_dispatch_sgr_256 (struct input_ctx *, int,
					     u_int *);
     static void input_csi_dispatch_sgr_rgb (struct input_ctx *, int,
					     u_int *);
  ...
  while (off < len)
    {
      ...
      /* Find the transition. */
      if (ictx->state != state ||
	  itr == NULL || ictx->ch < itr->first || ictx->ch > itr->last)
	{
...
input_osc_parse_colour (const char *p)
{
  ...
  if ((len == 12 && sscanf (p, "rgb:%02x/%02x/%02x", &r, &g, &b) == 3) ||
    ...
    colour = colour_join_rgb (r >> 8, g >> 8, b >> 8);
	   ...
	   y >= 0 && y <= 1 && k >= 0 && k <= 1)
    {
      colour = colour_join_rgb ((1 - c) * (1 - k) * 255,
				(1 - m) * (1 - k) * 255,
				(1 - y) * (1 - k) * 255);
    }
  else
