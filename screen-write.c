...
static struct screen_write_citem *screen_write_collect_trim (struct
							     screen_write_ctx
							     *, u_int, u_int,
							     u_int, int *);
...
void
screen_write_hline (struct screen_write_ctx *ctx, u_int nx, int left,
		    int right)
{
  struct screen *s = ctx->s;
  struct grid_cell gc;
...
/* Draw a horizontal line on screen. */
void
screen_write_vline (struct screen_write_ctx *ctx, u_int ny, int top,
		    int bottom)
{
  struct screen *s = ctx->s;
  struct grid_cell gc;
 ...
 */
void
screen_write_preview (struct screen_write_ctx *ctx, struct screen *src,
		      u_int nx, u_int ny)
{
  struct screen *s = ctx->s;
  struct grid_cell gc;
...
screen_write_cell (struct screen_write_ctx *ctx, const struct grid_cell *gc)
{
  ...
  if ((~s->mode & MODE_WRAP) &&
      width > 1 && (width > sx || (s->cx != sx && s->cx > sx - width)))
    return;
...
screen_write_combine (struct screen_write_ctx *ctx,
		      const struct utf8_data *ud, u_int * xx)
{
  struct screen *s = ctx->s;
  struct grid *gd = s->grid;
...
void
screen_write_setselection (struct screen_write_ctx *ctx, u_char * str,
			   u_int len)
{
  struct tty_ctx ttyctx;
