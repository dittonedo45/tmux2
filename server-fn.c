...
server_redraw_session_group (struct session *s)
{
  ...
  if ((sg = session_group_contains (s)) == NULL)
    ...
    {
      TAILQ_FOREACH (s, &sg->sessions, gentry) server_redraw_session (s);
    }
}
...
server_status_session_group (struct session *s)
{
  ...
  if ((sg = session_group_contains (s)) == NULL)
    ...
    {
      TAILQ_FOREACH (s, &sg->sessions, gentry) server_status_session (s);
    }
}
...
server_renumber_all (void)
{
  ...
  RB_FOREACH (s, sessions, &sessions) server_renumber_session (s);
}
  ...
  switch (remain_on_exit)
      ...
      if (WIFEXITED (wp->status))
	{
	  screen_write_nputs (&ctx, -1, &gc,
			      ...
			      WEXITSTATUS (wp->status), tim);
	}
      else if (WIFSIGNALED (wp->status))
	{
	  screen_write_nputs (&ctx, -1, &gc,
			      "Pane is dead (signal %s, %s)",
			      sig2name (WTERMSIG (wp->status)), tim);
	}
