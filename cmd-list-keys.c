  ...
  while (bd != NULL)
    {
	  ...
	  KEYC_IS_MOUSE (bd->key) || bd->note == NULL || *bd->note == '\0')
	{
...
cmd_list_keys_print_notes (struct cmdq_item *item, struct args *args,
			   const char *tablename, u_int keywidth,
			   key_code only, const char *prefix)
{
  ...
  while (bd != NULL)
    {
	  ...
	  ((bd->note == NULL || *bd->note == '\0') && !args_has (args, 'a')))
	{
	  bd = key_bindings_next (table, bd);
	  continue;
	}
	...
	{
	  status_message_set (tc, -1, 1, 0, "%s%s%s", prefix, tmp, note);
	}
      else
  ...
  for (entryp = cmd_table; *entryp != NULL; entryp++)
    {
	  ...
	  (strcmp (entry->name, command) != 0 &&
	   (entry->alias == NULL || strcmp (entry->alias, command) != 0)))
	continue;
