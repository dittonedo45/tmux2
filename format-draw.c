...
format_is_type (struct format_range *fr, struct style *sy)
{
  if (fr->type != sy->range_type)
    return (0);
  if (fr->type == STYLE_RANGE_WINDOW && fr->argument != sy->range_argument)
    return (0);
  return (1);
}
...
/* Fix range positions. */
static void
format_update_ranges (struct format_ranges *frs, struct screen *s,
		      u_int offset, u_int start, u_int width)
{
...
format_draw_put (struct screen_write_ctx *octx, u_int ocx, u_int ocy,
		 struct screen *s, struct format_ranges *frs, u_int offset,
...
format_draw_put_list (struct screen_write_ctx *octx,
		      u_int ocx, u_int ocy, u_int offset, u_int width,
		      struct screen *list, struct screen *list_left,
		      struct screen *list_right, int focus_start,
...
format_draw_none (struct screen_write_ctx *octx, u_int available, u_int ocx,
		  u_int ocy, struct screen *left, struct screen *centre,
		  struct screen *right, struct screen *abs_centre,
		  struct format_ranges *frs)
{
  ...
  format_draw_put (octx, ocx, ocy, right, frs,
		   ...
		   right->cx - width_right, width_right);
  ...
  format_draw_put (octx, ocx, ocy, centre, frs,
		   ...
		   - width_centre / 2,
		   centre->cx / 2 - width_centre / 2, width_centre);
  ...
  format_draw_put (octx, ocx, ocy, abs_centre, frs,
		   (available - width_abs_centre) / 2, 0, width_abs_centre);
}
...
format_draw_left (struct screen_write_ctx *octx, u_int available, u_int ocx,
		  ...
		  struct screen *right, struct screen *abs_centre,
		  struct screen *list, struct screen *list_left,
		  struct screen *list_right, struct screen *after,
		  int focus_start, int focus_end, struct format_ranges *frs)
{
   ...
   */
  while (width_left +
	 width_centre + width_right + width_list + width_after > available)
    {
      if (width_centre > 0)
	width_centre--;
  ...
  format_draw_put (octx, ocx, ocy, after, frs,
		   width_left + width_list, 0, width_after);
		    ...
		    int focus_start, int focus_end, struct format_ranges *frs)
{
   ...
   */
  format_draw_put (octx, ocx, ocy, centre, frs,
		   middle - width_list / 2 - width_centre, 0, width_centre);
   ...
   */
  format_draw_put (octx, ocx, ocy, after, frs,
		   middle - width_list / 2 + width_list, 0, width_after);
		   ...
		   int focus_start, int focus_end, struct format_ranges *frs)
{
  ...
  format_draw_put (octx, ocx, ocy, after, frs,
		   ...
		   after->cx - width_after, width_after);
  ...
  format_draw_put_list (octx, ocx, ocy, available - width_list -
			width_after, width_list, list, list_left, list_right,
...
format_draw_absolute_centre (struct screen_write_ctx *octx, u_int available,
			     u_int ocx, u_int ocy, struct screen *left,
			     struct screen *centre, struct screen *right,
			     struct screen *abs_centre, struct screen *list,
			     struct screen *list_left,
			     struct screen *list_right, struct screen *after,
			     int focus_start, int focus_end,
			     struct format_ranges *frs)
{
  u_int width_left, width_centre, width_right, width_abs_centre;
   ...
   */
  format_draw_put (octx, ocx, ocy, centre, frs,
		   middle - width_centre, 0, width_centre);
  ...
  format_draw_put_list (octx, ocx, ocy, abs_centre_offset, width_list,
			list, list_left, list_right, focus_start, focus_end,
			frs);
  ...
  switch (list_align)
    ...
    case STYLE_ALIGN_ABSOLUTE_CENTRE:
      ...
      format_draw_absolute_centre (octx, available, ocx, ocy, &s[LEFT],
				   &s[CENTRE], &s[RIGHT], &s[ABSOLUTE_CENTRE],
				   &s[LIST], &s[LIST_LEFT], &s[LIST_RIGHT],
				   &s[AFTER], focus_start, focus_end, &frs);
      break;
    }
