...
static void
server_client_overlay_timer (__unused int fd, __unused short events,
			     void *data)
{
...
have_event:
  ...
  if (m->statusat != -1 &&
      y >= (u_int) m->statusat && y < m->statusat + m->statuslines)
    {
...
server_client_loop (void)
{
  ...
  RB_FOREACH (w, windows, &windows) server_client_check_window_resize (w);
...
/* Resize timer event. */
static void
server_client_resize_timer (__unused int fd, __unused short events,
			    void *data)
{
...
static void
server_client_repeat_timer (__unused int fd, __unused short events,
			    void *data)
{
...
server_client_check_redraw (struct client *c)
{
  ...
  tty->flags =
    (tty->flags & ~(TTY_BLOCK | TTY_FREEZE | TTY_NOCURSOR)) | flags;
...
server_client_dispatch_identify (struct client *c, struct imsg *imsg)
{
   ...
   */
  if ((~c->flags & CLIENT_EXIT) &&
      !cfg_finished && c == TAILQ_FIRST (&clients))
    start_cfg ();
}
