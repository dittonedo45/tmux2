...
cmd_display_menu_get_position (struct client *tc, struct cmdq_item *item,
			       struct args *args, u_int * px, u_int * py,
			       u_int w, u_int h)
{
  ...
  if (top != -1)
	...
	ranges = &tc->status.entries[0].ranges;
      ...
      if (sr != NULL)
	{
	  format_add (ft, "popup_window_status_line_x", "%u", sr->start);
	  if (position == 0)
	    {
	...
	{
	  format_add (ft, "popup_status_line_y", "%u", tty->sy - lines);
	}
    }
  else
    top = 0;
  ...
  if (event->m.valid)
	...
	format_add (ft, "popup_mouse_centre_x", "%ld", n);
      ...
      if (n + h >= tty->sy)
	{
	  format_add (ft, "popup_mouse_centre_y", "%u", tty->sy - h);
	}
      else
...
cmd_display_menu_exec (struct cmd *self, struct cmdq_item *item)
{
  ...
  if (!event->m.valid)
    flags |= MENU_NOMOUSE;
  if (menu_display (menu, flags, item, px, py, tc, target, NULL, NULL) != 0)
    return (CMD_RETURN_NORMAL);
