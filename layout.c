...
static u_int
layout_new_pane_size (struct window *w, u_int previous,
		      struct layout_cell *lc, enum layout_type type,
		      u_int size, u_int count_left, u_int size_left)
{
		       ...
		       enum layout_type type, int size)
{
  ...
  count = 0;
  TAILQ_FOREACH (lcchild, &lc->cells, entry) count++;
...
layout_spread_cell (struct window *w, struct layout_cell *parent)
{
  ...
  number = 0;
  TAILQ_FOREACH (lc, &parent->cells, entry) number++;
  if (number <= 1)
    return (0);
  status = options_get_number (w->options, "pane-border-status");
