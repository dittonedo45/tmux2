...
#line 406 "cmd-parse.y"
	...
	cmd_prepend_argv (&(yyval.command)->argc, &(yyval.command)->argv,
			  (yyvsp[0].token));
...
#line 507 "cmd-parse.y"
	...
	(yyval.arguments).argv =
	  xreallocarray (NULL, 1, sizeof *(yyval.arguments).argv);
...
#line 514 "cmd-parse.y"
      {
	cmd_prepend_argv (&(yyvsp[0].arguments).argc,
			  &(yyvsp[0].arguments).argv, (yyvsp[-1].token));
	free ((yyvsp[-1].token));
...
cmd_parse_and_insert (const char *s, struct cmd_parse_input *pi,
		      struct cmdq_item *after, struct cmdq_state *state,
		      char **error)
{
...
cmd_parse_and_append (const char *s, struct cmd_parse_input *pi,
		      struct client *c, struct cmdq_state *state,
		      char **error)
{
  struct cmd_parse_result *pr;
  struct cmdq_item *item;
...
struct cmd_parse_result *
cmd_parse_from_buffer (const void *buf, size_t len,
		       struct cmd_parse_input *pi)
{
  static struct cmd_parse_result pr;
  struct cmd_parse_input input;
...
cmd_parse_from_arguments (int argc, char **argv, struct cmd_parse_input *pi)
{
  ...
  for (i = 0; i < argc; i++)
	...
	new_argc++;
      ...
      if (new_argc != 0)
	{
	  cmd_log_argv (new_argc, new_argv, "%s: at %u", __func__, i);
  ...
  if (last != argc)
    {
      ...
      if (new_argc != 0)
	{
	  cmd_log_argv (new_argc, new_argv, "%s: at %u", __func__, last);
