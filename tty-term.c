...
{
  return (tparm
	  ((char *) tty_term_string (term, code), a, 0, 0, 0, 0, 0, 0, 0, 0));
}

const char *
tty_term_string2 (struct tty_term *term, enum tty_code_code code, int a,
		  int b)
{
  return (tparm
	  ((char *) tty_term_string (term, code), a, b, 0, 0, 0, 0, 0, 0, 0));
}

const char *
tty_term_string3 (struct tty_term *term, enum tty_code_code code, int a,
		  int b, int c)
{
  return (tparm
	  ((char *) tty_term_string (term, code), a, b, c, 0, 0, 0, 0, 0, 0));
}
...
{
  return (tparm
	  ((char *) tty_term_string (term, code), (long) a, 0, 0, 0, 0, 0, 0,
	   0, 0));
}
...
{
  return (tparm
	  ((char *) tty_term_string (term, code), (long) a, (long) b, 0, 0, 0,
	   0, 0, 0, 0));
}
  ...
  switch (term->codes[code].type)
    ...
    case TTYCODE_STRING:
      ...
      xsnprintf (s, sizeof s, "%4u: %s: (string) %s",
		 code, tty_term_codes[code].name, out);
      break;
    case TTYCODE_NUMBER:
