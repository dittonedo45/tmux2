...
clients_calculate_size (int type, int current, struct client *c,
			...
			int (*skip_client) (struct client *, int, int,
					    struct session *,
					    struct window *), u_int * sx,
			u_int * sy, u_int * xpixel, u_int * ypixel)
{
  struct client *loop;
  u_int cx, cy, n = 0;
...
default_window_size_skip_client (struct client *loop, int type,
				 __unused int current, struct session *s,
				 struct window *w)
{
  /*
		     ...
		     int type)
{
  ...
  if (!clients_calculate_size (type, 0, c, s, w,
			       default_window_size_skip_client, sx, sy,
			       xpixel, ypixel))
    {
      ...
      if (sscanf (value, "%ux%u", sx, sy) != 2)
	{
	  ...
	  *sy = 24;
      ...
      log_debug ("%s: using %ux%u from default-size", __func__, *sx, *sy);
    }
...
recalculate_size_skip_client (struct client *loop, __unused int type,
			      int current, __unused struct session *s,
			      struct window *w)
{
  /*
...
recalculate_size (struct window *w, int now)
{
  ...
  changed = clients_calculate_size (type, current, NULL, NULL, w,
				    recalculate_size_skip_client, &sx, &sy,
				    &xpixel, &ypixel);
...
recalculate_sizes_now (int now)
{
  ...
  RB_FOREACH (w, windows, &windows) recalculate_size (w, now);
}
