...
tty_start_tty (struct tty *tty)
{
  ...
  tio.c_lflag &=
    ~(IEXTEN | ICANON | ECHO | ECHOE | ECHONL | ECHOCTL | ECHOPRT | ECHOKE |
      ISIG);
  tio.c_cc[VMIN] = 1;
  tio.c_cc[VTIME] = 0;
...
{
  if ((tty->term->flags & TERM_NOAM) &&
      tty->cy == tty->sy - 1 && tty->cx + len >= tty->sx)
    len = tty->sx - tty->cx - 1;
...
tty_update_mode (struct tty *tty, int mode, struct screen *s)
{
  ...
  if ((changed & ALL_MOUSE_MODES) && tty_term_has (tty->term, TTYC_KMOUS))
    {
      /*
...
int
tty_window_offset (struct tty *tty, u_int * ox, u_int * oy, u_int * sx,
		   u_int * sy)
{
  *ox = tty->oox;
  *oy = tty->ooy;
...
/* What offset should this window be drawn at? */
static int
tty_window_offset1 (struct tty *tty, u_int * ox, u_int * oy, u_int * sx,
		    u_int * sy)
{
  struct client *c = tty->client;
  struct window *w = c->session->curw->window;
...
tty_update_client_offset (struct client *c)
{
  ...
  if (ox == c->tty.oox &&
      oy == c->tty.ooy && sx == c->tty.osx && sy == c->tty.osy)
    return;

  log_debug ("%s: %s offset has changed (%u,%u %ux%u -> %u,%u %ux%u)",
	     __func__, c->name, c->tty.oox, c->tty.ooy, c->tty.osx,
...
static int
tty_clamp_line (struct tty *tty, const struct tty_ctx *ctx, u_int px,
...
static int
tty_clamp_area (struct tty *tty, const struct tty_ctx *ctx, u_int px,
		u_int py, u_int nx, u_int ny, u_int * i, u_int * j, u_int * x,
		u_int * y, u_int * rx, u_int * ry)
{
  u_int xoff = ctx->rxoff + px, yoff = ctx->ryoff + py;
  ...
  if (!tty_fake_bce (tty, defaults, bg))
    {
      ...
      if (px == 0 &&
	  px + nx >= tty->sx &&
	  py + ny >= tty->sy && tty_term_has (tty->term, TTYC_ED))
	{
	  tty_cursor (tty, 0, py);
...
void
tty_draw_line (struct tty *tty, struct screen *s, u_int px, u_int py,
...
tty_cmd_reverseindex (struct tty *tty, const struct tty_ctx *ctx)
{
  ...
  if (ctx->bigger ||
      ...
      (!tty_term_has (tty->term, TTYC_RI) &&
       !tty_term_has (tty->term, TTYC_RIN)) || ctx->sx == 1 || ctx->sy == 1)
    {
      tty_redraw_region (tty, ctx);
      return;
    }
...
static void
tty_cursor_pane (struct tty *tty, const struct tty_ctx *ctx, u_int cx,
		 u_int cy)
{
  ...
  if (cy == thisy)
      ...
      if ((u_int) abs (change) > cx && tty_term_has (term, TTYC_HPA))
	{
	  ...
	  goto out;
	       ...
	       tty_term_has (term, TTYC_CUB) && !tty_use_margin (tty))
	{
	  ...
	  goto out;
	       ...
	       tty_term_has (term, TTYC_CUF) && !tty_use_margin (tty))
	{
  ...
  if (wp->flags & PANE_STYLECHANGED)
    {
      ...
      style_add (&wp->cached_active_gc, oo, "window-active-style", NULL);
