...
void
grid_reader_start (struct grid_reader *gr, struct grid *gd, u_int cx,
		   u_int cy)
{
  gr->gd = gd;
  gr->cx = cx;
...
grid_reader_cursor_left (struct grid_reader *gr, int wrap)
{
  ...
  if (gr->cx == 0 && gr->cy > 0 &&
      (wrap || grid_get_line (gr->gd, gr->cy - 1)->flags & GRID_LINE_WRAPPED))
    {
...
grid_reader_cursor_next_word (struct grid_reader *gr, const char *separators)
{
  ...
  do
    {
      while (gr->cx > xx || grid_reader_in_set (gr, separators) == expected)
	{
...
void
grid_reader_cursor_next_word_end (struct grid_reader *gr,
...
void
grid_reader_cursor_previous_word (struct grid_reader *gr,
...
int
grid_reader_cursor_jump_back (struct grid_reader *gr,
			      const struct utf8_data *jc)
{
  struct grid_cell gc;
  u_int px, py, xx;
