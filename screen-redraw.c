...
static void
screen_redraw_border_set (struct window_pane *wp, int pane_lines,
...
/* Draw a border cell. */
static void
screen_redraw_draw_borders_cell (struct screen_redraw_ctx *ctx, u_int i,
				 u_int j)
{
  struct client *c = ctx->c;
  struct session *s = c->session;
  ...
  if (cell_type == CELL_TOPBOTTOM &&
      (c->flags & CLIENT_UTF8) && tty_term_has (tty->term, TTYC_BIDI))
    isolates = 1;
  else
    isolates = 0;
...
screen_redraw_draw_borders (struct screen_redraw_ctx *ctx)
{
  ...
  TAILQ_FOREACH (wp, &w->panes, entry) wp->border_gc_set = 0;
...
/* Draw one pane. */
static void
screen_redraw_draw_pane (struct screen_redraw_ctx *ctx,
			 struct window_pane *wp)
{
  struct client *c = ctx->c;
  struct window *w = c->session->curw->window;
  ...
  for (j = 0; j < wp->sy; j++)
      ...
      else
	{
      ...
      tty_draw_line (tty, s, i, j, width, x, y, &defaults, wp->palette);
    }
}
