...
int
popup_display (int flags, struct cmdq_item *item, u_int px, u_int py,
	       u_int sx, u_int sy, const char *shellcmd, int argc,
	       char **argv, const char *cwd, struct client *c,
	       struct session *s, popup_close_cb cb, void *arg)
{
  ...
  pd->job = job_run (shellcmd, argc, argv, s, cwd,
		     ...
		     JOB_NOWAIT | JOB_PTY | JOB_KEEPWRITE, pd->sx - 2,
		     pd->sy - 2);
