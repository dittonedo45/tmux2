...
status_timer_start_all (void)
{
  ...
  TAILQ_FOREACH (c, &clients, entry) status_timer_start (c);
}
...
status_redraw (struct client *c)
{
  ...
  if (o == NULL)
      ...
      for (i = 0; i < lines; i++)
	{
	  ...
	  if (!force &&
	      sle->expanded != NULL && strcmp (expanded, sle->expanded) == 0)
	    {
	      free (expanded);
	      continue;
	    }
 ...
 */
static int
status_prompt_translate_key (struct client *c, key_code key,
			     key_code * new_key)
{
...
static void
status_prompt_menu_callback (__unused struct menu *menu, u_int idx,
