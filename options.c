...
static int
options_array_cmp (struct options_array_item *a1,
...
RB_GENERATE_STATIC (options_array, options_array_item, entry,
		    options_array_cmp);
...
options_free (struct options *oo)
{
  ...
  RB_FOREACH_SAFE (o, options_tree, &oo->tree, tmp) options_remove (o);
  free (oo);
}
...
options_scope_from_name (struct args *args, int window,
			 const char *name, struct cmd_find_state *fs,
...
options_scope_from_flags (struct args *args, int window,
			  struct cmd_find_state *fs, struct options **oo,
			  char **cause)
{
  struct session *s = fs->s;
  struct winlink *wl = fs->wl;
...
options_from_string_choice (const struct options_table_entry *oe,
			    struct options *oo, const char *name,
			    const char *value, char **cause)
{
  const char **cp;
  int n, choice = -1;
...
options_from_string (struct options *oo, const struct options_table_entry *oe,
		     const char *name, const char *value, int append,
		     char **cause)
{
  ...
  if (oe != NULL)
    {
      if (value == NULL &&
	  oe->type != OPTIONS_TABLE_FLAG && oe->type != OPTIONS_TABLE_CHOICE)
	{
...
options_push_changes (const char *name)
{
  ...
  if (strcmp (name, "pane-border-status") == 0)
    {
      RB_FOREACH (w, windows, &windows) layout_fix_panes (w, NULL);
    }
  RB_FOREACH (s, sessions, &sessions) status_update_cache (s);
